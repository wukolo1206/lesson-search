...
- 建立專案紀錄（共5個）：
  - 啟發潛能之星申請（115年度國語）
  - 115典範社群（科技化學習扶助社群，社群領導人）
  - AI教育年會投稿（2026，學扶數學知識圖譜系統）
  - 閱讀教學策略查詢系統、字音形系統、超能競賽出題、115介聘申請（原有）
- 刪除 `D:\test ch\114啟發潛能之星申請\`（使用者確認可刪）
- 整合 Obsidian：PROFILE.md 第10節指向 Obsidian Dashboard；Obsidian `artifacts/個人資料.md` 建立指標筆記
- 建立三個 GitHub 私人 repo 備份：
  - `lesson-search`：D:\test ch\
  - `beike-ai`：D:\備課ai\ 程式碼
...
| 系統 | 負責什麼 |
|------|---------|
| Obsidian Dashboard | 所有專案完整清單（技術細節、部署連結、狀態） |
| PROFILE.md | 申請用自介素材、亮點摘要 |
| 專案紀錄/_專案摘要.md | 申請用完整描述（成效、功能、創新） |
| _inbox/ | 所有新文件的唯一入口 |
...
- obsidian-vault repo 排除了 備課ai Junction（由 beike-ai repo 另行備份）
- PROFILE.md 第10節專案清單已改為指向 Obsidian，只留申請用亮點摘要
