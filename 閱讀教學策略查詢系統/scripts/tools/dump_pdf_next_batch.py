...
OUTPUT_FILE = r'd:\test ch\閱讀教學策略查詢系統\output\pdf_batch_next.txt'
...
def main():
    ...
    print(f"剩餘 {len(missing_ids)} 筆資料缺漏。")
    if not missing_ids:
        print("所有資料皆已補齊！")
        return
    
    # 取前 8 筆作為下一批
    batch_size = min(8, len(missing_ids))
    batch_ids = missing_ids[:batch_size]
    print(f"本批次處理名單: {batch_ids}")
    ...
    for target_id in batch_ids:
        target_pdf = None
        for f in all_pdfs:
            if f.startswith(f"{target_id}-") or f.startswith(f"{target_id} ") or f.startswith(f"{target_id}－") or f.startswith(f"{target_id}_"):
                target_pdf = f
                break
            
            # 避免使用 'in' 導致 5-2-1 誤判 5-2-10
            # 加入精確的檔名開頭比對或確保 target_id 後面不是數字
            import re
            if re.match(rf"^{target_id}[^\d]", f):
                target_pdf = f
                break

            if target_id == f.replace('.pdf', ''):
                target_pdf = f
                break
